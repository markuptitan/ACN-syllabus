...
5. INSERT the records displayed in the tables below into the tables you created in step 2.

6. Document what information is stored in your database. Be sure to specify what information is kept in which table, and which keys link the records between tables. This file needs to be in a `.md` format.
...
**NB! Be sure to label your answers by putting the question above the code that answers it, submitting a wall of code with no explanations makes it difficult to review.**

#### Queries:

7. SELECT ALL records from the Customers table.

8. SELECT records only from the FirstName column in the Customers table.

9. Show the full name of the customer whose CustomerID is 1.

10. UPDATE the record for CustomerID = 1 in the Customers table so that the name is "Lerato Mabitso".
...
13. Return the MAXIMUM payment made in the Payments table.
...
19. Return the sum of the amounts in the Payments table.
...
- The following files should be present:
  ...
  - A new `.md` file documenting what information is in which table and how the tables are linked, specifying which keys link the records between tables. The `README.md` file should not be edited, it serves it's own purpose.
